...
for k = 1:totalMice
    ...
    for j = 1:4
        ...
        load(matName);
        [numRed, avgRed, manualPointsIr] = vmhManualCount(Ir,hVmhVlPatchMask); %red only
        %save data for each section:
%         matName = [filepathRoot, '\', mouseNum, '\', mouseNum, '_', num2str(j), '_redCount2.mat'];
%         save(matName, 'numRed', 'avgRed', 'manualPointsIr');
        close all; %prevent large number figures
    end
