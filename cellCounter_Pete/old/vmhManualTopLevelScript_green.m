...
for k = 1:totalMice
    ...
    for j = 1:4
        ...
        load(matName);
        [numGreen, avgGreen, manualPointsIg] = vmhManualCount(Ig,hVmhVlPatchMask); %green only
        %save data for each section:
        matName = [filepathRoot, '\', mouseNum, '\', mouseNum, '_', num2str(j), '_greenCount2.mat'];
        save(matName, 'numGreen', 'avgGreen', 'manualPointsIg');
        close all; %prevent large number figures
    end
