function [patchMask, maskCoord, ellip, xMin, xMax, yMin, yMax] = sectionSetRoiEllipseJpeg(IchooseRoi)
% sectionSetRoiEllipseJpeg: function to read image and pick ROIs and set thresholds in order to quantify fluorescence
% Jason Keller, 5/16/2017
%
% inputs:
...
hFig = figure;
hIm = imshow(IchooseRoi); %create image handle and show image
set(gcf, 'Name','Choose approximate ellipse center point:');
...
xMax = round(uint16(roiXLim(2)))-1;
yMin = round(uint16(roiYLim(1)));
yMax = round(uint16(roiYLim(2)))-1;
...
hCenter = impoint(hAxes);  %first draw center point of ellipse manually
set(hCenter, 'Visible', 'off');
pos = getPosition(hCenter);
xCenter = pos(1);
yCenter = pos(2);

[patchMask, maskCoord, ellip] = rotateEllipse(xCenter, yCenter, hAxes, hFig, hIm);                            
