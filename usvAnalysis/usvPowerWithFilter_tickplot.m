...
wavPath = '\\172.29.164.29\Jingyi Chen\Dropbox\Jingyi Data backup\Behavior data\CHR2 experiment\2019-10 PAG vGat ChR2\10s light stimulation all\25hz 10s\';
...
AUCpowerARRAY = [];     
nameList = [];
% usvPowerPerSampleSmoothMat = zeros(0,179963); % for 4 mins WT SMUF analysis only
% usvPowerPerSampleSmoothMat = zeros(0,89852); % for 2 mins WT SMUF analysis only
% usvPowerPerSampleSmoothMat = zeros(1,22463);  % hardcode with amount of validSamples for 30s long files
usvPowerPerSampleSmoothMat = zeros(0,37463);  % hardcode with amount of validSamples % for 50s long video, added by Jingyi on 2019/8/7
...
usvPowerPerSample = sum(signalCleaned,1) ./ (highFreq-lowFreq);
...
% usvPowerPerSampleSmoothMat= usvPowerPerSampleSmoothMat();
AUCAvg = mean(AUCpowerARRAY);

% %% Plot the average signal -ZG %Wong time domain. Use Pete's code for all
% time fig
% 
...
imagesc(usvPowerPerSampleSmoothMat(2:end,:)); % throw a matrix into imagesc, boom heatmap
...
% x=[89100 89100];
% y=[0 totalMice]; 
% line(x,y, 'Color', [0 0.3 1])% For 4 mins WT behavior
% x=[4455 4455];       % plot the stimulus end at 6 seconds (4455) 
% x=[7425 7425];       % plot the stimulus end at 10 seconds (7425)  
% % x=[11137.5 11137.5];       % plot the stimulus end at 15 seconds (11137.5) 
% % x=[14805 14805];       % plot the stimulus end at 20 seconds (14805) 
% % y=[0 totalMice+1];   % should run from bottom (0) to top (totalMice)
% line(x,y, 'Color', [0 0.3 1])
...
ylabel({'Sample'});
% xlabel('Time (seconds)');
xlabel('Time (mins)');% for 4 mins SMUFF behavior
c = colorbar;
caxis([1 12])  % set the colorbar limits from 0-12 dB/Hz
c.Label.String = {'USV power';'(dB/Hz)'};
...
set(gca, 'XTick', [1:3712.5:37463], 'XTickLabel', xticklabels);
% xticklabels({'0','1','2','3','4'});
% set(gca, 'XTick', [1:44550:178200], 'XTickLabel', xticklabels);
