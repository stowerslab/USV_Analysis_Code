...
% wavPath = 'C:\Users\chenj\Dropbox (Scripps Research)\Jingyi Data backup\Behavior data\WT behaviors\2018-2 WT USVs\2018-3-1 WT USV MALE URINE\2MINS TRIMED AUDIO\';
% wavPath =  'E:\Jingyi\Dropbox (Scripps Research)\Jingyi Data backup\Behavior data\CHR2 experiment\2020-6 vGluT2 ChR2 LPOA\2020-5-22 vGluT2-ChR2-T2\1HZ\';
% wavPath =  'E:\Jingyi\Dropbox (Scripps Research)\Jingyi Data backup\Behavior data\DREADD experiment\2020-8 Esr DREADD control\4mins trimed data with female\Saline day4\';
wavPath =  '\\172.29.164.29\Jingyi Chen\Dropbox\Jingyi Data backup\Behavior data\2020-6 chr2 PAG-FP\Trimed ChR2 data\1Hz\';
files = dir('*.wav');
plotandsaveindividual = 1; % only if want to plot USVs and save individual Mat files
...
USVStats = struct(stat1,[], stat2,[], stat3,[], stat4,[], stat5,[], stat6,[], stat7,[]);

usvPwrPerSampleAll = [];
timepointUSVs = [];
nameList = {};
% usvPowerPerSampleSmoothMat = zeros(1,179963); % for 4 mins WT analysis only
% usvPowerPerSampleSmoothMat = zeros(1,89963); % for 2 mins analysis only
usvPowerPerSampleSmoothMat = zeros(1,18713); % For 25s 
...
zthresh = 1.5; %1.5 with urine and 1.7 with live female
...
usvPowerPerSample = sum(signalCleaned,1) ./ (highFreq-lowFreq); %average power across frequencies (so mean dB from ~40-80kHz, over temporal smoothing filter); do NOT normalize for now
...
%     firstcall= NaN ; 
%     latencytocall(j) = NaN;
%Add by JC on 12/2/2019 for WT USV analysis only
    firstcall= 120; 
    latencytocall(j) = 120;
end
% latencytocall(j) = firstcall-5; % in seconds only for ChR2 stimulation
latencytocall(j) = firstcall;
...
% boundedline(T(1:validSamples), mean(usvPowerPerSampleSmoothMat(:, :, 1)), std(usvPowerPerSampleSmoothAvg(:, :, 1)),'-r');%red color for female
...
boundedline(T(1:validSamples), mean(usvPowerPerSampleSmoothMat(:, :, 1)), std(usvPowerPerSampleSmoothMat(:, :, 1))/sqrt(size(usvPowerPerSampleSmoothMat, 1)),'-b');%blue color for male or saline day for DREADD
% orange= [ 0.9100 0.4100 0.1700];
% boundedline(T(1:validSamples), mean(usvPowerPerSampleSmoothMat(:, :, 1)), std(usvPowerPerSampleSmoothMat(:, :, 1))/sqrt(size(usvPowerPerSampleSmoothMat, 1)),'cmap', orange);%orange color for DREADD GAQ
...
axis([xLims(1) xLims(2) -0.15 4]);     % -0.05 to go slightly below lowest call, 2 should be above highest call for all other ChR2
...
% Sortindex = latencytocall';
% [~,idx] = sort(Sortindex, 'ascend');
...
xlabel('Time (seconds)');
c = colorbar;
caxis([0 10])  % set the colorbar limits from 0-5 dB
...
xticklabels({'0','5','10','15','20','25'}); %for 25 seconds long
% xticklabels({'0','20','40','60','80','100','120'}); %for 120 seconds long
% set(gca, 'XTick', [1:3712.5:37125], 'XTickLabel', xticklabels) %for 50s longs
% set(gca, 'XTick', [1:14850:89100], 'XTickLabel', xticklabels) %for 50s long
set(gca, 'XTick', [1:3742.6:18713], 'XTickLabel', xticklabels) %for 25s long
