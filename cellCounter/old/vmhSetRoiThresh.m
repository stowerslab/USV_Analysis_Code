function [roiPatchArray, vmhXLim, vmhYLim , GfpThresh, GfpThreshImage, TdtThresh, TdtThreshImage, Ig, Ir, Ibwater] = vmhSetRoiThresh(filepathIn, mouseNum, sectionNum)
%% vmhSetRoiThresh: function to read image and pick ROIs and set thresholds in order to quantify viral expression in the VMH
% Jason Keller, 5th Dec 2013
%
% This script does all the manual steps first (picking ROIs and
% thresholds), do that time-consuming processing can be done all at once
% 
% TO DO: 
...
%       (3) VMHc
% (B)   images: Ig, Ir, Ibwater (for vmhQuant.m)
% (C)   thresholds for GFP & TDT channels (for vmhCountCells.m)
...
%% ROI selection
% manually perform selection of 3 ROIs using the Hoechst channel: (VMHc can be computed)
numRoi = 2;
roiPhrases = {'(1) Select total VMH ROI:'; '(2) Select VMH_dm ROI using centered line:'};
...
%make alternate X& Y limits to plot only VMH ROI:
...
pause on; pause(3); pause off;  %pause for 3 seconds
...
subimageIg = uint8(Igfilt(yMinAlt:yMaxAlt, xMinAlt:xMaxAlt) .* (255/maxPixelValue));
[GfpThresh, GfpThreshImage] = threshToolMod(subimageIg);
GfpThresh = GfpThresh / 255*maxPixelValue; % convert back to 12-bit relevance

% TDT:
Irfilt = imfilter(Ir, hNucleusDisk, 'replicate'); 
% make 8-bit smaller image to feed into threshTool:
% subimageIr = uint8(Irfilt .* (255/maxPixelValue));
subimageIr = uint8(Irfilt(yMinAlt:yMaxAlt, xMinAlt:xMaxAlt) .* (255/maxPixelValue));
[TdtThresh, TdtThreshImage] = threshToolMod(subimageIr);
TdtThresh = TdtThresh / 255*maxPixelValue; % convert back to 12-bit relevance
