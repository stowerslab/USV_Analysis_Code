function [patchMask, vertPoints, ellip] = rotateEllipse(xCenter, yCenter, hAxes, hFig, hIm) %mainfunction
...
ellip.theta = 60;  % initial angle from x-axis in degrees
ellip.a = 100;  % initial major semiaxis length 
ellip.b = 50; % initial minor semiaxis length 
...
xVert = get(hPatch, 'XData');
yVert = get(hPatch, 'YData');
patchMask = poly2mask(xVert, yVert, size(get(hIm,'CData'),1), size(get(hIm,'CData'),2));  %create binary mask from area enclosed by X & Y data

% Once manipulation is finished, draw another patch for half ellipse boundary (+/- 90 degrees in parametric equation):
if ellip.left % if on left of image
    vertPoints = 90:180/(numVertPoints/3):270; %+/- 90 degrees from 180
else % if on right of image
    vertPoints = -90:180/(numVertPoints/3):90; %+/- 90 degrees
end
% [x, y] = makeEllipse(ellip.a, ellip.b, ellip.center, ellip.theta, vertPoints);
